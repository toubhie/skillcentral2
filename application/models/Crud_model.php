...
class Crud_model extends CI_Model
{
        ...
        if ($previous_data == 0) {
            if ($this->input->post('parent') == 0) {
                ...
                if ($_FILES['category_thumbnail']['name'] == "") {
                    ...
                    $this->load->helper('azure_helper');
                    upload_video_to_azure($_FILES['category_thumbnail']['tmp_name'], 'uploads/thumbnails/category_thumbnails/' . $data['thumbnail']);
                }
            }
    ...
    public function add_course($param1 = "")
    {
        ...
        foreach ($course_media_files as $course_media => $size) {
            if ($_FILES[$course_media]['name'] != "") {
                ...
                $this->load->helper('azure_helper');
                upload_video_to_azure($_FILES[$course_media]['tmp_name'], 'uploads/thumbnails/course_thumbnails/' . $course_media . '_' . get_frontend_settings('theme') . '_' . $course_id . '.jpg');
            }
        }
    ...
    public function add_lesson()
    {
        ...
        if ($lesson_type == 'video') {
            ...
            $this->load->helper('azure_helper');
            upload_video_to_azure($tmp_video_file, $video_file_path);
            ...
            if ($attachment_type == 'iframe') {
                ...
                if ($_FILES['attachment']['name'] == "") {
                    ...
                    $this->load->helper('azure_helper');
                    upload_video_to_azure($_FILES['attachment']['tmp_name'], 'uploads/lesson_files/' . $uploadable_file);
                }
            }
        }
        ...
        if ($_FILES['thumbnail']['name'] != "") {
            ...
            $this->load->helper('azure_helper');
            upload_video_to_azure($_FILES['thumbnail']['tmp_name'], 'uploads/thumbnails/lesson_thumbnails/' . $inserted_id . '.jpg');
        }
    }
        ...
        if ($lesson_type == 'video') {
            ...
            if ($lesson_provider == 'youtube' || $lesson_provider == 'vimeo') {
                ...
                if ($_FILES['thumbnail']['name'] != "") {
                    ...
                    $this->load->helper('azure_helper');
                    upload_video_to_azure($_FILES['thumbnail']['tmp_name'], 'uploads/thumbnails/lesson_thumbnails/' . $lesson_id . '.jpg');
                }
            } else {
                $this->session->set_flashdata('error_message', get_phrase('invalid_lesson_provider'));
                redirect(site_url(strtolower($this->session->userdata('role')) . '/course_form/course_edit/' . $data['course_id']), 'refresh');
